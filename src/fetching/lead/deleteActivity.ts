...
const useDeleteActivity = () => {
  ...
  const deleteActivity = async (leadID: string | null, activityID: string): Promise<boolean> => {
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      const res = await fetch("api/lead/deleteActivity", {
        method: "DELETE",
        body: formData
      })
  ...
  return { loading, data, error, deleteActivity };
};
