...
const useGetListQuotation = () => {
  const [loading, setLoading] = useState<boolean>(false);


  const getAllQuotation = async (): Promise<{
    ...
    try {
      const response = await fetch("/api/quotation/getAllQuotation", {
        method: "GET",
      });
...
export default useGetListQuotation;