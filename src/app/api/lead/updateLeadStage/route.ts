...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const forwardStageID = formData.get("forwardStageID");

    if (!leadID || !forwardStageID) {
      return NextResponse.json(
        { code: 400, message: "Fill in lead ID or forward stage ID" },
        { status: 400 }
      )
    }

    const resBackend = await fetch(`${path}/${leadID}/stages?stageId=${forwardStageID}`, {
      method: "PATCH",
      headers: {
